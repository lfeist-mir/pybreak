...
ENEMY_COLORS = ['red3','sienna1','khaki2','chartreuse3','blue2','blueviolet','magenta']
ENEMY_LINES = 8
ENEMY_PER_LINE = 8
...
class GameManager():
    ...
    def __init__(self, width, height):
        ...
        self.paddle = Paddle(13*self.block_size, 33*self.block_size, self.block_size)
        ...
        for i in range(ENEMY_LINES):
            for j in range(ENEMY_PER_LINE):
                self.enemies.append(Brick(3*self.block_size+(i*3)*self.block_size,
                                          ...
                                          color=ENEMY_COLORS[(i*8+j)%len(ENEMY_COLORS)]
                                          ))
        self.ball = Ball(self.paddle.x+round(self.paddle.width/2),
